				...
				?>
					...
					<table class="table table-bordered table-shopping-cart">
						...
						<tbody>
							...
							?>
								<tr>
									<td><img src="<?= base_url('assets/images/kalender/') . $value['kalender_foto'] ?>" alt="foto" style="width: 100%"></td>
									<td><?= $value['kalender_ukuran'] ?></td>
									<td><?= $value['kalender_cetakisicover'] ?> </td>
									<td><?= $value['kalender_jumlahhalaman'] ?> </td>
									<td><?= $value['kalender_jumlah'] ?> </td>
									<td style="text-align: right"> Rp.<?= nominal($value['kalender_total']) ?></td>
									<td><a class="fa fa-close table-shopping-remove" href="<?= base_url('hapus/kalender/' . $value['kalender_id']) ?>" onclick="return confirm('Hapus Pesanan? ')"></a></td>
								</tr>
							<?php
							endforeach;
							?>
